...
def build_conv_evictions():
    # Prefer the dedicated cache (all PACT BBLs, 2017+) generated by pact_conv_evictions.py.
    # Fall back to pact_executions.csv only if the preferred cache is missing.
    execs_path = ANALYSIS_DIR / "pact_executions_conv.csv"
    if not execs_path.exists():
        execs_path = ANALYSIS_DIR / "pact_executions.csv"
    if not execs_path.exists():
        print("  no execution cache found; skipping conv_evictions.json")
        print("  run: python pact_conv_evictions.py --refresh")
        return None
